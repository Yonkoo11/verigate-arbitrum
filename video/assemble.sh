...
ORDER=(01-hook 02-problem 03-engine 04-registry 05-activity 06-gate 07-close)
LEAD=0.5        # audio starts 0.5s into each segment
FADE=0.2

for name in $ORDER; do
  # seg = the clip's content window (recorded at the TAIL, past load/connect).
  seg=$(python3 -c "import json;print(json.load(open('clips/segments.json'))['$name'])")
  clipdur=$(ffprobe -v error -show_entries format=duration -of csv=p=0 clips/$name.webm)
  start=$(python3 -c "print(round(max(0.0, $clipdur - $seg), 2))")
  ...
  ffmpeg -y -loglevel error -ss $start -i clips/$name.webm -i captions/$name.png -i audio/$name.mp3 \
    -filter_complex "$fg" -map "[v]" -map "[a]" -t $seg -r 30 \
    -c:v libx264 -preset medium -pix_fmt yuv420p -c:a aac -ar 44100 $WORK/seg-$name.mp4
  echo "seg $name (${seg}s, tail from ${start}s)"
done
